 ...
 * at a time.
 */
public abstract class SingleRequestService implements ProfileService {

    @Override
    public final ImmutableList<Profile> findAllByName(Iterable<String> names) {
    ...
    @Override
    public final void findAllByUuid(Iterable<UUID> uuids, Predicate<Profile> consumer) {
        for (UUID uuid : uuids) {
            Profile profile = findByUuid(uuid);
            if (profile != null) {
                consumer.test(profile);
